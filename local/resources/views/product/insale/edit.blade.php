...
@section('content')
    ...
    <div class="box">
        ...
        <form action="{{ url('product/insale/step1',[$product->product_id]) }}" class="form_submit" method="post">
        <!-- /.box-header -->
        <div class="box-body">
            ...
            <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-7">
                        ...
                        <select class="form-control required" msg="请选择供应商！"  name="supplier_id" id="supplier_id">
                            <option value="">—请选择—</option>
                            @foreach($supplier_list as $val)
                                <option @if(isset($product->supplier_id)&&$val->supplier_id == $product->supplier_id) selected @endif value="{{ $val->supplier_id }}">{{ $val->shortname }}</option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="col-sm-4">
                        ...
                        <select class="form-control required" msg="请选择一级分类" id="class_1" onchange="a(1, this);">
                            <option value="">—请选择—</option>
                            @foreach($class as $val)
                                <option @if($val->class_id == $product->parent_class) selected @endif value="{{ $val->class_id }}">{{ $val->class_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-sm-4">
                        <label><b>二级分类：</b></label>
                        <select class="form-control required"  msg="请选择二级分类" id="class_2" name="select_class" onchange="a(2, this);">
                            <option value="">—请选择—</option>
                            @foreach($class_sub as $val)
                                <option @if($val->class_id == $product->class_id) selected @endif value="{{ $val->class_id }}">{{ $val->class_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <label><b>主标题：</b></label>
                        <input type="text" class="form-control required" msg="请填写主标题" style="width:80%;" name="product_name" value="{{ $product->product_name }}" placeholder="">
                    </div>
                ...
                <div class="row" style="margin-top:8px;">
                    <div class="col-sm-12">
                        <label><b>副标题：</b></label>
                        <input type="text" class="form-control" style="width:80%;" name="remarks" placeholder=""  value="{{ $product->remarks }}">
                    </div>
                </div>
