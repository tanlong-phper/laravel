            ...
            <div class="wrap">
                <ul class="nav nav-tabs">
                    <li @if(Request::get('dzd_type')==1 or empty(Request::get('dzd_type'))) class="active" @endif><a
                                href="/balance/done?dzd_type=1" target="_self">T+1</a></li>
                    <li @if(Request::get('dzd_type')==2) class="active" @endif><a
                                href="/balance/done?dzd_type=2" target="_self">T+7</a></li>
                </ul>
                <div class="margin-bottom-20">
                    <form action="{{url('balance/done')}}" method="get">
                ...
                <form class="form-horizontal js-ajax-form" method="post" novalidate="novalidate">
                    <table class="table table-hover table-bordered">
                        ...
                        @foreach($data as $key=>$item)
                            <tr class="one" data-dzd-id="{{$item->dzd_id}}" data-dzd-type="{{$item->dzd_type}}">
                                ...
                                <td class="text-center">{{$item->finance_name}}</td>
...
@section('js')
    <script>
        ...
        $('body').on('click', '.view', function () {
            if ($(this).parents('.one').attr('data-dzd-type') == 1) {
                var url = '/balance/review/t_plus1_review_detail?dzd_id=' + $(this).parents('.one').attr('data-dzd-id'); //iframe的url
            } else if ($(this).parents('.one').attr('data-dzd-type') == 2) {
                var url = '/balance/review/t_plus7_review_detail?dzd_id=' + $(this).parents('.one').attr('data-dzd-id'); //iframe的url
            } else {
                layer.msg('结算单类型不合法');
                return false;
            }
