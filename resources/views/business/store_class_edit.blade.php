	...
	<div class="wrap js-check-wrap">
		...
		<form method="post" class="form-horizontal js-mobile-add-form" enctype="multipart/form-data">
			<div class="row-fluid">
				<table class="table table-bordered">
					<tr>
						<th>分类名称</th>
						<td>
							<input type="text" style="width:400px;" name="type_name" id="type_name"  value="{{$data->type_name or '' }}" placeholder="请输入店铺分类名称"/>
						</td>
					</tr>
					...
					<tr>
						<th>分类logo</th>
						<td>
							<img id='thumb' src="{{$data->type_logo or '/static/admin/assets/images/default-thumbnail.png'}}" style="height:120px;" />
							<input type="file" name="image" onchange="upload(this)" />
							<input type="hidden" name="pic" value="{{$data->type_logo or ''}}" />
						</td>
					</tr>
			...
			<div class="form-actions">
				...
				<a class="btn" href="{{asset('system/store_class_list')}}">返回</a>
			</div>
		</form>
	</div>
					...
					$.post("/system/{{$action}}",data, function (msg) {
						...
						if (msg.status == 1) {
							layer.confirm(msg.info, {
								...
								@if($action == 'add')
										window.location.href="{{asset('system/store_class_list')}}";
								@else
										window.location.href="{{asset('system/store_class_list')}}";
								@endif
							});
