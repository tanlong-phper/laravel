...
<div class="wrap">
    <ul class="nav nav-tabs">
        <li><a href="/balance/review" target="_self">T+1复审</a></li>
        <li class="active"><a href="javascript:;">T+7复审</a></li>
    </ul>
    <div class="margin-bottom-20">
        <form action="{{url('balance/review/t_plus7_review')}}" method="get">
            ...
            <div id="search-area">
                ...
                <div class="cl pd-5 bg-1 bk-gray mt-20">
                    ...
                    <select name="keyname" class="form-control">
                        <option value="dzd_id" @if(Request::get('keyname')=='dzd_id') selected @endif>结算单ID</option>
                        <option value="table_name" @if(Request::get('keyname')=='table_name') selected @endif>结算单名称</option>
                        <option value="trial_name" @if(Request::get('keyname')=='trial_name') selected @endif>初审人</option>
                    </select>
        ...
        <table class="table table-hover table-bordered">
            ...
            @foreach($data as $key=>$item)
                <tr class="one" data-dzd-id="{{$item->dzd_id}}">
                    <td class="text-center"><input name="id-all" type="checkbox" value="{{$item->dzd_id}}" class="details_id"></td>
                    <td class="text-center">{{$key+1}}</td>
                    <td class="text-center">{{$item->dzd_id}}</td>
                    <td class="text-center">
                        <a class="view">{{$item->table_name}}</a><br>
                        {{$item->date_str}}
                    </td>
                    <td class="text-center">
                        @if($item->unusual==0)
                            正常结算
                        @elseif($item->unusual==1)
                            异常结算
                        @else
                            未定义
                        @endif
                    ...
                    <td class="text-center">{{$item->settle_price}}</td>
...
<script>
    ...
    $('body').on('click','.view',function () {
        ...
        layer.open({
            ...
            shade: 0.8,
            area: ['99%', '99%'],
            content: '/balance/review/t_plus7_review_detail?dzd_id='+$(this).parents('.one').attr('data-dzd-id'), //iframe的url
            cancel:function () {
